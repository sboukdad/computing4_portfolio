...
// verify no-seed constructor generates different sprites each time
BOOST_AUTO_TEST_CASE(TestRandomSeedVariation) {
    // create two builders without specifying a seed
    sb::SpriteBuilder builder1("spritebuilder/rogue.txt");
    sb::SpriteBuilder builder2("spritebuilder/rogue.txt");
...
// verify randomize() changes the sprite
BOOST_AUTO_TEST_CASE(TestRandomizeChangesSprite) {
    sb::SpriteBuilder builder("spritebuilder/rogue.txt", 12345);

    sf::Vector2u size = builder.windowSize();

    // capture first sprite
    sf::RenderTexture rt1({size.x, size.y});
    rt1.clear(sf::Color::Transparent);
    rt1.draw(builder);
    rt1.display();
    ...
    // randomize and capture second sprite
    builder.randomize();
    sf::RenderTexture rt2({size.x, size.y});
    rt2.clear(sf::Color::Transparent);
    rt2.draw(builder);
